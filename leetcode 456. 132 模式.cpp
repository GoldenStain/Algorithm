...
public:
  ...
  bool find132pattern(vector<int> &nums) {
    ...
    for (int k = 1; k < n - 1; k++) {
      ...
      // 在k这里，我们用的是k+1 ~ n-1
      // 范围内的数字，我们k要往后移动了，那我们查找的区间也要相应地向后收缩。
...
// solution 2
// 这个解法虽然形式上是单调栈，但本质上借用的不是栈上的信息
// 反而是借助了在出栈时产生的信息，在栈定形的时候
// 这些“有效信息”反而已经被pop掉了
// 因此需要我们手动去维护和记录这个过程
class Solution {
public:
  // 枚举1
  bool find132pattern(vector<int> &nums) {
    // 题解说“如果我们从左到右枚举 1 的下标 i，那么 j,k
    // 的下标范围都是减少的，这样就不利于对它们进行维护。因此我们可以考虑从右到左枚举
    // i”我的理解是，如果i从左边开始，那么我们需要先求出完整的j和k集合，然后再一步一步地缩小它，这样更不好维护；如果i从右边开始，那么j和k的可选范围都是增加的，这有利于我们维护；回看枚举2的解法，我们枚举i，j和k一个正向增长，一个逆向增长，那无论我们从左还是从右开始，都是这种局面，那也就无所谓从哪边开始了。
    // 我们用单调栈来维护2，每次有新元素的时候，可以直接把单调栈内部所有小于该元素的值都弹出，因为2越大，越能容纳可能的1；弹出的过程中，更新一下最大的2
    // max_candidate的值； 这里隐含了一个条件：
    // 只要我们的max_candidate不再是初始值，就说明：
    // 我们已经通过上面的操作更新过单调栈了，也就意味着已经有满足3,
    // 2条件的数对了（新元素对应3，被弹出的元素对应2）
    ...
    for (int i = n - 2; i >= 0; i--) {
      ...
      if (nums[i] < max_candidate)
        return true;
      // 小坑，等于nums[i]的不能出栈，因为nums[i]不大于栈顶，就不能构成3 2
      // ，就没法更新答案
...
public:
  ...
  bool find132pattern(vector<int> &nums) {
    ...
    for (int i = 1; i < n; i++) {
      if (i > 1) {
        // check
        auto it_1 = upper_bound(stk_1.begin(), stk_1.end(), nums[i],
                                greater<int>()),
             it_3 = lower_bound(stk_3.begin(), stk_3.end(), nums[i],
                                greater<int>()) -
                    1;
        if (it_1 != stk_1.end() && it_3 != stk_3.end()) {
          // 一开始我有这样的顾虑：
          // idx(it_1) == idx(it_3)的时候会不会误判？
          // 毕竟有可能这俩都是某个之前的nums[i]
