...
2的case需要while是因为，我们可能会换一个2过来到当前的位置上，要一直换到不是2为止；
...
2025/6/2出现在每日一题当中，二刷，并且添加了比较详细的注释。
...
**总结**
DFS做法和BFS做法相比，前者是把左右两颗子树都展开成链表之后，再连接起来，先成型再连接
而BFS做法，则是先把左右两颗子树连接起来，然后在向下继续连接的过程中，逐渐成型，先连接再成型
有点类似于归并排序和快速排序的区别
...
要达到极限的运行速度，我们需要：
1. 减少stl容器的size()属性的调用，直接用一个变量“缓存”下来。

2. 减少if语句的使用。

3. 这道题有两种递推公式，一种是类似状态机，一种是用`dp[i-1]`和`dp[i-2]`来共同更新`dp[i]`，为了追求极限速度，我们需要使用后者，因为后者需要用到和更新的变量更少，运行起来也就更快。
...
在这个循环判定条件中：`while (stk.size() && heights[stk.back()] >= heights[i])` 这里取不取等号最后的结果都是对的：

![柱状图中最大的矩形1](柱状图中的最大矩形.png)

**终极优化**

为什么我们原来需要两重循环和两个额外数组才能计算出答案呢？
我们原来使用的公式：`ans = std::max(ans, (right_[i] - left_[i] - 1) * heights[i]);`

实际上，这里的left和right数组都没有必要显示地记录下来。

我们已经知道：在这个递增的单调栈当中，每个数字入栈的时候，栈顶就是左边第一个比它小的；出栈的时候，准备入栈的数字就是右边第一个比它小的。

我们的入手点就在于：每个数字入栈时的栈顶，也就是紧挨着它的前面一个。即我进行一次出栈操作之后，新的栈顶，就是原栈顶的“左边的第一个比它小的数”，此时再配合右边的条件，就能在不开辟额外空间、只使用一次循环的情况下把最终答案统计出来。

**优化版本当中的细节**

在需要开辟额外空间的普通解法当中，我们提到，在出栈判定条件中是否加等号都Ok，最终答案会在不同的时机被计算出来。
但是在优化版本中，因为我们有连续的两次出栈操作，所以情况有了些许不同（注意题目条件，`height[i] >= 0`）：
1. 不带等号：`while(!_stack.empty() && heights[_stack.back()] > heights[i])`
    此时，我们维护的单调栈是非严格递增的，而我们padding的数据和题目原有的数据，最小值都是0，此时栈底至少会有一个我们padding的0，因此能保证正确。
2. 带等号：`while(!_stack.empty() && heights[_stack.back()] >= heights[i])`
    此时，我们维护的单调栈是严格递增的，因为我们padding的值和原有数据最小值都是0，那么我们padding的值在`height[i]`取到0的时候，就会被pop掉，此时我们有两种策略：一. 加一个if语句，防止内存访问越界；二.在开头padding一个-1，最后仍padding一个0，这样的话我们padding的-1就永远不会出栈，能保证栈永不为空。
...
### update
添加了基于灵神思路的最优解，直接把84.题封装成一个函数调用。
主要变动：
1. 两头padding-1.
2. 计算的时候当前矩形是开区间`(left, right)`，来减少特判。
3. 不再以列为底边，而是以行为底边，减少循环次数。

这版必须用>=号而不是>号，因为我们是开区间，所以必须保证栈上的数字是严格递增的，否则计算面积会偏小。
...
如果要求LIS个数的话，可以先用二分求出最长的长度，然后再遍历一次，利用树状数组求出方案数。
...
先转换成前缀和，然后再在前缀和数组上做滑动窗口。

最开始的代码把前缀和跟滑动窗口分了两个循环写，效率非常低，尝试将两个操作合并到一个循环当中后，

### 复杂解法

复杂解法里面直接把整个sum用一个变量来维护，导致更新逻辑十分麻烦，所以时间上还是很慢，不过因为没有使用前缀和数组，所以空间的开销已经很低了

### 最优解

用total和cnt来组合出sum，简化更新逻辑，加快计算

另外，这道题有两种逻辑：
1. hh每次移动一步，tt一直移动，直到不满足条件为止
2. tt每次移动一步，hh一直移动，直到满足条件为止

推荐2.中的写法，因为和其他常规的滑动窗口是一致的喵，越一致，越方便记忆和掌握，越不容易写错

## Leetcode 2071. 你可以安排的最多任务数目

一开始我就想到了排序+贪心的做法，但是如果要直接通过贪心来获得最优解，是很困难的，我尝试了几种贪心策略，都被反例推翻了。

因此我们可以换个更简单的贪心：我们不利用贪心本身去寻找最优解，而是假定最优解之后，再用贪心去验证是否可行。

这个“假定”的过程，就是我们的老朋友“二分答案”了。

之后，题目的解法就类似于“Leetcode 2560. 打家劫舍 IV”了，先二分出一个目标值，然后去检验这个目标值是否可行。

**思路分析**

官方题解给出了贪心策略的正确性证明，这里给出一定补充：

为什么不用药的情况下是优先用高体力值工人，而用药的情况下却是优先用低体力值工人？

因为在**不用药**的情况下，我们的贪心目标是：尽量对每个工人“物尽其用”，让高体力值工人做低开销工作是一种浪费；

在**用药**的情况下，我们的贪心目标是：尽量节省药，减少药的消耗。

例如，tasks是`[9, 10]`，workers是`[8, 9]`，我们只有一个能量为1的药，如果我们优先用了高体力工人，那么最后只能完成一个任务；如果先用低体力工人，就能完成两个。

即：如果我们先用了服药后的低体力工人来处理当前的高难度任务，我们就省下了一个高体力工人，在遇到下一个没这么高的任务时，这个高体力工人可能在不服药的情况下就可以胜任，如果我们在当前就消耗掉了那个高体力工人，那后面的任务交给低体力工人，他就算服药了，也没法搞定。

综上，在服药的情况下，我们应该优先使用低体力工人。

**实现细节**

要注意的是，静态构造ws队列的方法是错误的，样例3能够hack掉，我们需要动态地去维护ws
（静态指在正式解决问题之前就一口气构造好ws，动态指一边求解一边维护ws）

## Leetcode 1128. 等价多米诺骨牌对的数量

来自于每日一题。

先根据哈希把所有多米诺骨牌分桶，然后每个桶内部，都可以两两组成一个等价对。

要记得开long long。

## Leetcode 1920. 基于排列构建数组

朴素解法自不必说，我们只考虑不消耗额外空间的进阶做法。

一开始我想的是类似交换环那样的做法：

遍历每个位置，逐个在while循环中进行swap，但是这种做法找不到严谨的时间复杂度证明。

而且交换环的做法本身就是需要额外空间的（用来保存排序好的数组，即带有目标位置信息的数组副本），那此时还不如使用朴素解法，因此我们舍弃该思路。

看了下官方的题解，使用的做法类似于`Leetcode 41. 缺失的第一个正数`，即借助数组本身的空间，在不丢失信息的情况下，进行原地哈希。

这道题的关键就在于数字的范围比较小，可以用1000进制来无损地保存信息。
...
我一开始的处理方法是给`Point`加一个cost字段，实际上这是可以省略的，因为：

我们每一步的开销都是正数，所以不可能走回头路，因此可以根据当前起点的横纵坐标之和，来判断当前一共走了几步，据此确认cost的值。

但是这个小优化并不能优化算法的性能。

lc上其他更快的做法，都是在入队时遇到终点就直接返回，但据我所知DJ算法并不能保证此时是最短路。

但是**这道题有一个特殊性质**：

所有的路径，他们虽然会经过不同的格点，但是他们走的总距离都是相等的，经过平移重组后，都等价于沿着横边竖边各走一边，这种情况天然限制了这样的case：
...
虽然DP看起来比记忆化搜索更加高级，但实际上它不一定更优，因为记忆化搜索**只会涉及到符合到合法的、有意义的状态**，但是DP会计算很多**无用的，不参与答案贡献的无效状态**，因此记忆化搜索反而有可能更快。

但是考虑到函数调用的开销比单纯的执行循环大得多，所以还是得具体问题具体分析。

一开始把MOD写错了，debug半天没看出来。。。

### 记忆化搜索与DP的重要区别

记忆化搜索是从上至下，所以每次总数与总个数是不断变少的，即我们已经考虑过的数字不用再考虑了，已经选好了。

而DP是从下至上，所以每次总数与总个数都要先加上当前数字的贡献，表示：我们之前没遍历到这里，所以不用考虑，现在遍历到了，要把这个数字也纳入考虑范围了。

# Leetcode 3335. 字符串转换后的长度 I

第一眼想要进行取模直接快速求长度，但是因为z会变成ab，但是ab回到z所需要的步数又不相同，因此直接取模是没法解决这个问题的。

于是我手动画了若干步骤进行模拟，发现case越变越复杂，没有办法继续用一个简洁的方式来描述每一步产生的转化，到这里基本可以确定**这道题没法用模拟来解决**了。

因此，我们需要用DP的思想，设计状态，用一个数字来表示一个集合，但是这个状态我没设计出来，最后还是看了题解。

在朴素线性复杂度解法的基础上，我们可以进一步使用快速幂进行优化。

斐波那契数列的矩阵快速幂优化需要同时把$f_i$和$f_{i-1}$都放进矩阵里面，是因为求$f_{i+1}$的时候要同时用到这两个值，但是这个问题当中，所有的$f_{i+1}$的值都只依赖$f_{i}$，所以我们只需要把$f_i$存进矩阵里就好了，然后按照递推公式构造更新矩阵即可。

深夜写的时候脑子短路了，不小心把f写成了单位矩阵，整体思路没啥难点。

不过这道题的原始数据规模并不太大，而且矩阵乘法会带来函数调用和计算矩阵乘的额外开销（最坏情况下是$26^3$）
...
做的时候注意力相当不集中，一方面想到了操作只对路径首尾的点有效，另一方面也想到了应该设计`dp[u][2]`的状态，但是没有推出正解。

**错误原因**

1. 对于思路一，把题目想复杂了，理解成了一个路径相互覆盖的问题，甚至还想用树上差分去做，这个思路的关键是：树上任意两点之间都是可达的，而又因为操作只对路径上首尾的点有效，所以相当于每次任选两个点操作。 由此可以设计出官方题解的方法一和三

2. 树上DP，脑子短路了，状态设计出来了，但是方程没有推出来。状态转移方程见官方题解二.注意因为`dp[u][0]`和`dp[u][1]`的状态转移方程相互包含，所以转移顺序有讲究。

3. 类似树上dp的线性dp公式，抛弃了边的关系
...
因为不能引入额外的空间开销，所以不能用递归写法，我们可以手动去模拟递归的回溯过程。

最后可以发现，虽然理论复杂度是相同的，但是递归解法的实际耗时是迭代解法的三倍以上

## Leetcode 2616. 最小化数对的最大差值

这道题的问法一看就是很典型的二分答案然后去验证的问题。

验证时的贪心策略，就是尽量选取相差比较小的数字去配对。也就是排序之后相邻的两个元素。

官方题解提出了一个很直达本质的见解，即这道题和打家劫舍的二分版本是等价的。

我们既可以写dp（公式和打家劫舍二分版本那道题几乎一致），也可以直接写贪心（显然尽量选小的更优，就没有必要dp选取最优值了）
...
这道题可以用来辨析`push_back`和`emplace_back`.

## Leetcode 3405. 统计恰好有 K 个相等相邻元素的数组数目
...
这篇题解写的非常好
https://leetcode.cn/problems/is-subsequence/solutions/2813031/jian-ji-xie-fa-pythonjavaccgojsrust-by-e-mz22/392

## Leetcode 47. 全排列 II

1. 利用官方题解提供的这个判定条件`if (vis[i] || (i > 0 && nums[i] == nums[i - 1] && !vis[i - 1]))`，这个条件的本质是：
我们dfs的层数就表示当前正在选择排列当中的第几个数，对于全排列，假设我们的输入序列是`112`，为了方便理解，我们把两个1分别打上几号，整个序列表示为`1a1b2`。我们要保证的就是`1a1b2`和`1b1a2`不会被重复计入答案。
...
把二分区间的写法从**闭区间**改成**左闭右开**应该就能避免这个问题，但是因为当下的二分写法已经习惯很久了，所以还是不做修改为好.
...
**因为**前面的大于号保证了`g`能够被正确更新，而这里写成大于等于，应该是为了保证在初始值（`f`还是0的情况下）时，能够正确更新（物理意义下`f`最小只能是1，但刚初始化的时候`f`还是0）
...
我们规定向右是X轴正方向。x坐标越大越前面。
经过分析，可以发现这道题最核心的性质是：
1. 如果第i辆车，原本能追上前面若干辆车，那么他自己是否被追上，对这个状态没有任何影响。因为被追上的话，那么追上来的车一定比i车快，所以i车不管怎样都不会变慢（853和1776均用到）
2. 根据并查集的思想，我们可以把车队里最靠前的一辆车，作为这个车队的代表元素。（853）
3. 一个车队当中的车都是数轴上连续的一段

以上两点性质当中，帮助我们解题的核心性质是第二点，基于此，我们能针对“车队里**最靠前**的车”来建立单调栈。
更具体地说，如果i车能追上i+1车，当我们要求i车i+1车相遇时间的时候，以为i+1车的速度是变化的（追上前车以后变慢），从i车和i+1车之间入手，就很难捋清楚问题。
但是对于被i+1车追上的车来说（也就是栈上，i+1车下面的那辆车），他的速度从开始到被i+1车追上之后，都是没有变化过的，我们就可以转而计算i车和该车的追击问题，简化出"**两匀速车辆**的追及相遇问题"。


在853当中，做了一个简化，直接用**到达终点的时间**来作为快慢的评判标准，并且对于每个车队，只入栈代表元素。

**收获**
这两道题，让我更加深刻地认识到，**单调栈**对比起**单调队列**，后者是一种数据结构，使用起来相对固定；而**单调栈**实际上是一种思想，我们说一道题的做法是**单调栈**，可能并没有严格按照板子的形式去写，只是借用这种单调的特征/规律，从中提炼信息去解题。

从1776的代码就能看出来，因为这道题即便栈顶从数字上看合法了，也可能因为提前追上后方的车而变得不合法，如果保持单调栈的公式写法就会引入很丑陋的`goto`语句，所以这里我们采用了一种和板子完全不同的写法。

## 907.子数组的最小值之和

有点像蓝桥杯的某一道题，直接从题面的角度去计算答案时间复杂度太高，我们可以转而计算每个数字对最终答案的贡献。
很明显是用单调栈
要注意的是：
1. 贡献的计算方式：一个左端点和一个右端点能唯一确定一个子数组，因此只要用二者相乘即可。
2. 我们计算的区间是(L, R)，开区间
3. 为了防止相等数字的贡献被重复计算，我们令L位置为左边第一个小于当前数字的数，而R位置则为小于等于，这样的话，对于重复数字来说，最右边那一节区间的贡献，只会在后面的位置被计算，前面不会重复计入。
...
这道题的算法本质：
一种特化的**最短路**，一条路径的权值是**这条路径上的最大权值**，而对于一个节点来说，蓄水量取决于他到**水桶边界**的路径，我们知道一共有四个方向，根据木桶效应，一个节点的真实蓄水量应该是这四个方向上的**最小值**。
也就是说，这道题当中，**最短路**的定义是**该路径上权值的max**，而一个点对答案的贡献，则是**min{四个方向}(最短路长度)**。

这道题的**Dijkstra**和我们正常求解**Dijkstra**的写法不一样：
1. 正常的**Dijkstra**，是拿某个点扩展之后，才会标记vis
2. 这道题，入队的时候就标记vis了

顺便回顾一下**Dijkstra**的证明：https://zhuanlan.zhihu.com/p/570932634

注意：**入队**并不意味着加入集合**S**，vis置为true的时候，才是真正地加入了集合**S**。
如果入队的时候就把**vis**置为true，很容易被hack掉，acwing上面的样例就过不去。

那为什么这道题可以这么做呢？

本质上，我们不能在节点u刚入队时，就`vis[u] = true`，是因为当前虽然`dist[u]`局部最短，但还有可能，后续我们能通过其他点间接走过来，而这个间接路径可能比直接路径还要短；
但是当**节点u出队**的时候，他已经是整个堆上dist最小的节点了，已经不可能再通过其他路径来缩短了。
也就是说，在传统写法中，一个节点u，其`dist[u]`是可能被多次更新的。

但是这不适用于接雨水的情况下。

我们每次经过一个节点都会累加答案，如果这个节点会被重复更新，那答案显然就会偏大。

怎么避免这一点呢？ 我们一开始就把所有可能的起点加进来，那么每一步扩展出来的，都是所有可能路径中最短的，即真实最短路，那我们就能保证，**节点u入队时，其dist已经确定**了，即
我们可以在入队的时候就`vis[u] = true`，如此一来每个点只会入队一次，答案也就不会重复累计了。

## 文远知行2025.9.7笔试复盘

1. 第一题较简单，划分区间去逐个计算即可；要注意，题目给出的两个点没有确定上下顺序，而我是逐行计算的，所以需要先保证A点在B点的上方。

2. 第二题，本质上是转换问题形式，可以参考题解：https://www.luogu.com.cn/problem/solution/CF1542B
...
通过这道题可以回顾一下裴蜀定理。
要注意，python当中，字符串是不可变对象，因此不能用下标来改变某个位置上的字符，这道题需要先转换成int的list

## 2043. 简易银行系统

非常直白的模拟题，要注意检查输入是否合法。

## 480. 滑动窗口中位数

这道题没法用滑动窗口，因为滑动窗口只能保证单调性，却无法控制窗口中元素的数量，没办法精准地设计两个“对顶”队列。
因此这道题还是要用“对顶堆”来做，除了在push的时候要维护堆的元素个数外，在pop的时候也要维护堆的元素个数，保证每一时刻堆都是合法的。