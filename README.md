...
我们原来使用的公式：`ans = std::max(ans, (right_[i] - left_[i] - 1) * heights[i]);`对每个位置i都有效，但是因为right_数组的特殊性，导致我们需要先遍历一遍整个原数组，才能将其计算出来。
但事实上，每个合法的矩形，它的高度一定是取自两个较低的边界柱子，所以我们只需要在每个边界柱子上计算出这个矩形的贡献就行了，如此一来就省去了统计left_和right_所带来的时空开销。
